 ...
 * weex-vue-router 0.0.4 经过hiaming修改2处,用bundle来保存bundle.js参数
 */
...
const autuRouter = {
    ...
    push(vue_app, url) {
        if (this.isWeb()) {
            vue_app.$router.push(url);
        } else {
            const navigator = weex.requireModule('navigator');
            let bundleUrl = weex.config.bundleUrl;
            // var modal = weex.requireModule('modal')
            // modal.toast({
            //     message: '缓存的routes=' + JSON.stringify(this.routes),
            //     duration: 5.3
            // })
            let bundle = pathToBundle(url, this.routes);
            if (navigator) {
                console.log('真实的网址是:' + bundle);
                navigator.push({
                    'url': bundle,
                    'animated': 'true'
                }, function () {
    ...
    route() {
        const navigator = weex.requireModule('navigator');
        let bundleUrl = weex.config.bundleUrl;
        const route = bundleToPath(bundleUrl, this.routes);
        return {
            path: route.path,
            params: route.params,
            query: route.query,
    ...
    // install(Vue, ext) {
    //     console.log(ext.routes);
    //     console.log(ext.weex);
    //     let platform = weex.config.env ? weex.config.env.platform : weex.config.platform;
    //     if (platform.toLowerCase() == 'web')return;
    //     this.routes=ext.routes;
    //     var modal = weex.requireModule('modal')
    //     modal.toast({
    //       message: '设置缓存的routes='+ JSON.stringify( this.routes),
    //       duration: 5.3
    //     })
    // }
}
...
function pathToBundle(url, routes) {
    ...
    // if (typeof matchRule.bundle !== 'string' || matchRule.bundle.indexOf("/") !== 0) {
    //     matchRule.bundle = jsDomain + matchRule.path + '.js' //修复少数未设bundle的常规地址
    // }
    let componentPath = matchRule.bundle;
    for (let k in params) {
