...
const weexVueRouter = {
    install(Vue, ext) {
        ...
        const route = bundleToPath(bundleUrl, ext.routes.routes);
        Object.defineProperty(Vue.prototype, "$router", {
            value: {
                push(url){
                    // var modal = weex.requireModule('modal')
                    // modal.toast({
                    //   message: 'uuu跳到'+ JSON.stringify(ext.routes.routes),
                    //   duration: 5.3
                    // })
                    let bundle = pathToBundle(url, ext.routes.routes);
                    if (navigator) {
                        console.log(bundle);
                        navigator.push({
                            'url': bundle,
